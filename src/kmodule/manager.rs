...
fn add_module(name: String, info: elf_mod_info_s) {  //将模块加入hash表
    modules.lock().push((name,info));
}

fn del_module(name: String) {  //将模块移除hash表
	match hash_find(name)
	{
	    Some(idx) => { modules.lock().remove(idx); },
	    None => {},
	}
}

fn get_module(name: String)-> Option<elf_mod_info_s> {  //get名为name的模块信息
	match hash_find(name)
	{
	    Some(idx) => {
 ...
 */
fn module_loaded(name: String)-> bool {
	match hash_find(name)
	{
	    Some(_) => true,
	    None => false,
